...
double my_pow(double num, double m)//调用pow_f()和pow_i(),计算num的m次幂,是计算幂的入口
{
	...
	if (m<0) return 1 / my_pow(num, -m);//把指数小于0的情况转为1/num^-m计算
	if (m - (int)m == 0) return pow_i(num, m);/*当指数为浮点数是，分成整数和小数分别求幂，这是因为但底数较小式，用pow_f直接求幂误差大，所以分为指数的整数部分用pow_i,小数部分用pow_f求.*/
